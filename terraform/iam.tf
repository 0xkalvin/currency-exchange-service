...
resource "aws_iam_role" "server_task_iam_role" {
  name               = "${var.project}-${terraform.workspace}-server-task-role"
...
data "aws_iam_policy_document" "services_iam_policy_document" {
  statement {
    ...
    resources = [
      aws_sqs_queue.order_creation_queue.arn,
      aws_sqs_queue.order_settlement_queue.arn,
      aws_sqs_queue.movement_creation_queue.arn,
    ]
  }
  ...
  statement {
    ...
    resources = [
      aws_dynamodb_table.currency_exchange_table.arn,
      aws_dynamodb_table.balance_table.arn,
      "${aws_dynamodb_table.balance_table.arn}/index/*",
      "${aws_dynamodb_table.currency_exchange_table.arn}/index/*",
    ]
  }
}
